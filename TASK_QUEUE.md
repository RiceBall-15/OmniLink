...
#### 23. 代码清理和优化 ✅
- [x] 清理未使用的导入和变量（2026-05-13: im-api warnings 清零）
- [ ] 统一错误处理模式
- [x] 添加模块级文档注释（2026-05-13: 所有 crate 已添加）
...
#### 41. 错误处理统一 🔄
- [x] 统一所有服务的错误类型定义（使用 common::error 模式）
- [ ] 实现全局错误捕获中间件
- [x] 标准化 API 错误响应格式（统一使用 ApiResponse）
- [ ] 添加错误码枚举
...
#### 43. 日志和监控增强 ⏳
- [ ] 统一 tracing 日志格式（JSON 格式）
- [ ] 添加请求耗时中间件
- [ ] 实现 Prometheus metrics 端点
- [ ] 添加业务指标（消息数、用户数、API 调用数）

#### 44. 性能优化 ⏳
- [ ] 数据库连接池优化（配置最大连接数）
- [ ] Redis 缓存层实现（用户信息、会话列表缓存）
- [ ] 消息批量查询优化（避免 N+1 查询）
- [ ] WebSocket 连接池管理优化
